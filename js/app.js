(() => {
    ...
    function initSliders() {
        if (document.body.clientWidth < 991.98) {
            if (!swiperInstance) swiperInstance = new swiper_core_Swiper(".swiper", {
                modules: [ Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: "auto",
    ...
    window.addEventListener("load", (() => {
        initSliders();
        window.matchMedia("(min-width:991.98px)").addEventListener("change", (e => {
            if (e.matches) destroySliders(); else initSliders();
        }));
    }));
    class ScrollWatcher {
        constructor(props) {
    ...
    da.init();
    mouseSlider();
    window.addEventListener("resize", mouseSlider);
    function mouseSlider() {
        const sliders = document.querySelectorAll("[data-slider]");
        sliders.forEach((slider => {
            let container = slider.querySelector(".slider-card__image-container");
            const isRealMobile = isMobile.any() && window.innerWidth < 991.98;
            if (!container) {
                const originalImages = Array.from(slider.querySelectorAll("img"));
                if (!originalImages.length) return;
                container = document.createElement("div");
                container.classList.add("slider-card__image-container");
                originalImages.forEach(((img, index) => {
                    img.classList.add("slider-card__image-item");
                    if (index === 0) img.classList.add("slider-card__image-item--active");
                    container.appendChild(img);
                }));
                slider.innerHTML = "";
                slider.appendChild(container);
                const hoverZones = document.createElement("div");
                hoverZones.classList.add("hover-zones");
                ...
                container.appendChild(hoverZones);
                const indicatorsContainer = document.createElement("div");
                indicatorsContainer.classList.add("slider-card__indicators");
                originalImages.forEach(((_, i) => {
                    const indicator = document.createElement("div");
                    indicator.classList.add("slider-card__indicator-image");
                    if (i === 0) indicator.classList.add("slider-card__indicator-image--active");
                    indicatorsContainer.appendChild(indicator);
                }));
                slider.appendChild(indicatorsContainer);
            }
            const imageItems = container.querySelectorAll(".slider-card__image-item");
            const indicators = slider.querySelectorAll(".slider-card__indicator-image");
            let currentIndex = [ ...imageItems ].findIndex((img => img.classList.contains("slider-card__image-item--active")));
            const activateImage = index => {
                currentIndex = index;
                imageItems.forEach(((img, i) => {
                    img.classList.toggle("slider-card__image-item--active", i === index);
                }));
                indicators.forEach(((indicator, i) => {
                    indicator.classList.toggle("slider-card__indicator-image--active", i === index);
                }));
            };
            const oldHoverZones = container.querySelector(".hover-zones");
            if (oldHoverZones) oldHoverZones.remove();
            if (!isRealMobile) {
                const hoverZones = document.createElement("div");
                hoverZones.classList.add("hover-zones");
                imageItems.forEach(((_, i) => {
                    ...
                    zone.classList.add("hover-zone");
                    zone.dataset.index = i;
                    zone.addEventListener("mouseover", (() => activateImage(i)));
                    hoverZones.appendChild(zone);
                }));
                container.appendChild(hoverZones);
            }
            ...
            if (isRealMobile && imageItems.length > 1) {
                if (!prevBtn && !nextBtn) {
                    const newPrev = document.createElement("div");
                    const newNext = document.createElement("div");
                    newPrev.classList.add("slider-nav", "slider-nav--prev");
                    newNext.classList.add("slider-nav", "slider-nav--next");
                    newPrev.innerHTML = "&lsaquo;";
                    newNext.innerHTML = "&rsaquo;";
                    container.appendChild(newPrev);
                    container.appendChild(newNext);
                    newPrev.addEventListener("click", (e => {
                        e.stopPropagation();
                        const newIndex = (currentIndex - 1 + imageItems.length) % imageItems.length;
                        activateImage(newIndex);
                    }));
                    newNext.addEventListener("click", (e => {
                        e.stopPropagation();
                        const newIndex = (currentIndex + 1) % imageItems.length;
                        activateImage(newIndex);
                    }));
                }
            } else {
